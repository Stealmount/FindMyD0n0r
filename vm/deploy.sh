...
if [ ! -f "$ENV_FILE" ]; then
  ...
  sed -i "s|ADMIN_PORT=.*|ADMIN_PORT=6001|" "$ENV_FILE"
else
...
echo "[Firewall] Opening ports 3001, 5000, 6001…"
...
sudo iptables -I INPUT -p tcp --dport 6001 -j ACCEPT 2>/dev/null || true
...
echo "  Admin      → http://145.241.154.187:6001/"
